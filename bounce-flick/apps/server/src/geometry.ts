...
export function offsetPolylineSegments(points: Point[], offset: number): Segment[] {
  const offsetSegments = polylineSegments(points).map((segment) => offsetSegment(segment, offset));
  const segments: Segment[] = [];
...
export function segmentsYAt(
  ...
  for (const segment of segments) {
    const y =
      Math.abs(segment.to.x - segment.from.x) < EPSILON && Math.abs(x - segment.from.x) < EPSILON
...
export function segmentXStops(segments: Segment[], fromX: number, toX: number): number[] {
