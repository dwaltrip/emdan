...
const FINISH_DISTANCE_X: Range = [3600, WORLD_WIDTH - START_X - FINISH_RIGHT_MARGIN];
...
function createCorridorPlatform(
  ...
  const { anchorX, fillIndex, left: desiredLeft, minSurfaceY, topBuffer, width } = opts;
  ...
  const { left, right } = flatSpanAroundAnchor(
    ...
    (candidateLeft, candidateRight) => {
      const fit = platformFitRange(corridor, candidateLeft, candidateRight, height, topBuffer);

      return fit?.fits === true && (minSurfaceY === undefined || minSurfaceY <= fit.maxSurfaceY);
    },
  );
...
function growEdge(anchorX: number, stops: number[], edgeFits: (edge: number) => boolean): number {
  let edge = anchorX;
...
// the target.
function corridorFaceXStops(corridor: Corridor, fromX: number, toX: number): number[] {
  const stops = segmentXStops([...corridor.faces.ceiling, ...corridor.faces.floor], fromX, toX);
...
function createScatteredPlatforms(
  ...
  for (
    ...
    const platform = createScatteredPlatform(random, minLeft, maxRight, corridor, platforms.length);
...
function createScatteredPlatform(
  ...
  const leftTop = clamp(
    corridorCenterYAt(corridor, midX) +
      randomInt(random, -SCATTER_PLATFORM_VERTICAL_JITTER, SCATTER_PLATFORM_VERTICAL_JITTER),
    MIN_TRACK_TOP + 70,
    MAX_TRACK_TOP - 90,
  );
...
function isPlatformClear(candidate: Platform, platforms: Platform[], corridor: Corridor) {
  return (
    ...
    platforms.every((platform) => {
      ...
      const verticalNear =
        Math.abs(platformMidY(candidate) - platformMidY(platform)) < SCATTER_PLATFORM_CLEARANCE_Y;
...
function createCorridor(
  ...
  const slope = (finishPoint.y - startPoint.y) / (finishPoint.x - startPoint.x);
...
function createCorridorPath(
  ...
  appendCorridorLeg(random, path, { x: finishX, y: clamp(finishY, minY, maxY) }, minY, maxY);
  appendCorridorLeg(random, path, { x: endX, y: clamp(endY, minY, maxY) }, minY, maxY);
...
function appendCorridorLeg(
  ...
  while (target.x - current.x > CORRIDOR_STEP_X[1]) {
    ...
    const maxStep = Math.min(CORRIDOR_STEP_X[1], remainingX - CORRIDOR_STEP_X[0]);
  ...
  if (path[path.length - 1]!.x !== target.x || path[path.length - 1]!.y !== target.y) {
    path.push(target);
  }
}
...
function chooseCorridorPoint(
  ...
  const candidates: Array<Point & { weight: number }> = [
    weightedCorridorPoint(current, target, x, clamp(idealY, candidateMinY, candidateMaxY), idealY),
  ];

  for (let index = 1; index < CORRIDOR_CANDIDATE_COUNT; index += 1) {
    const y = clamp(
      idealY + randomInt(random, -CORRIDOR_RANDOM_WALK_Y, CORRIDOR_RANDOM_WALK_Y),
      candidateMinY,
      candidateMaxY,
    );
...
function weightedCorridorPoint(
  ...
  const targetOvershoot = Math.max(0, y - target.y - CORRIDOR_UPWARD_SOFT_LIMIT);
  const targetPenalty = Math.abs(y - idealY) / CORRIDOR_TARGET_PENALTY_Y;
  const upwardPenalty = (upwardExcess / CORRIDOR_UPWARD_SOFT_LIMIT) * CORRIDOR_UPWARD_PENALTY;
  const overshootPenalty =
    (targetOvershoot / CORRIDOR_UPWARD_SOFT_LIMIT) * CORRIDOR_UPWARD_PENALTY * 0.65;
...
function pickWeightedPoint(random: Random, candidates: Array<Point & { weight: number }>): Point {
...
function nearestCorridorSample(corridor: Corridor, x: number, y: number) {
  ...
  for (let index = 1; index < corridor.path.length; index += 1) {
    ...
    const progress = clamp(((x - from.x) * dx + (y - from.y) * dy) / segmentLengthSquared, 0, 1);
...
function surfaceYAt(platform: Platform, x: number) {
  const progress = clamp((x - platform.left) / (platform.right - platform.left), 0, 1);
